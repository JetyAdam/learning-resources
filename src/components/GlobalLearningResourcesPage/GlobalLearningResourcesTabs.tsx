...
interface GlobalLearningResourcesTabsProps {
  ...
  onSelect: (event: React.MouseEvent<any> | React.KeyboardEvent | MouseEvent, tabIndex: string | number) => void;
...
const GlobalLearningResourcesTabs: React.FC<GlobalLearningResourcesTabsProps> = ({
  ...
  return (
    ...
    >
      <Tab eventKey={0} title={<TabTitleText>All learning resources ({learningResourcesCount})</TabTitleText>} tabContentId="refTabResources" />
      <Tab eventKey={1} title={<TabTitleText>My bookmarked resources ({bookmarkedResourcesCount})</TabTitleText>} tabContentId="refTabBookmarks" />
    </Tabs>
  );
};
